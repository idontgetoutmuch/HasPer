...
{\em eExtConsInteger} if the constraint is extensible. This function takes five
inputs. The three required for {\em encodeNonExtConsInt} and the two -- actual and effective
-- extension constraints.
...
\begin{code}
prTest = BL.unpack $ BB.toLazyByteString $ selectOutput $ extractValue $ encode personnelRecord [] pr

pr = emp :*: t :*: num :*: hiredate :*: sp :*: Just cs :*: Empty

personnelRecord
    = BuiltinType
        (TAGGED (Application, 0, Implicit)
            (BuiltinType
                (SET
...
empNumber = BuiltinType
                (TAGGED (Application, 2, Implicit)
                    (BuiltinType INTEGER))
...
dateC = BuiltinType
			  (TAGGED (Context, 1, Explicit) date)
                        ...
                        (NoIntersection
                            (ElementConstraint
                                (P (FR (RootOnly
																	 		 		(UnionSet
                														(NoUnion
																						   (NoIntersection (ElementConstraint (S (SV (VisibleString "0123456789"))))))))))))
												(ElementConstraint (SZ (SC (RootOnly
																	 		 		 		 		 	 (UnionSet
                																		 		(NoUnion
...
soc = BuiltinType
			  (TAGGED (Context, 3, Implicit)
            (BuiltinType (SEQUENCEOF childInfo)))


childInfo = BuiltinType
                (SET
                    (AddComponent (MandatoryComponent (NamedType "name" name))
                        (AddComponent (MandatoryComponent (NamedType "dateOfBirth" dateC2))
                                            EmptySequence)))
                        ...
                        (NoIntersection
                            (ElementConstraint
                                (P (FR (RootOnly
																	 		 		(UnionSet
                														(NoUnion
																						   (NoIntersection (ElementConstraint (S (SV (VisibleString (['a'..'z'] ++ ['A'..'Z'] ++ ['-','.'])))))))))))))
												(ElementConstraint (SZ (SC (RootOnly
																	 		 		 		 		 	 (UnionSet
                																		 		(NoUnion
